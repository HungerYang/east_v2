...
def load_zip_file(file,fileNameRegExp='',allEntries=False):
    ...
    try:
        ...
        raise Exception('Error loading the ZIP archive {}'.format(file))    
...
def get_tl_line_values(line,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0):
    ...
    if LTRB:
        ...
        numPoints = 8;
        nums = line.split(',')
        points = list(map(float, nums[:8]))        
        points = list(map(int, points))
        if len(nums)>8:
            transcription = ','.join(nums[8:])
        #if withTranscription and withConfidence:
        ...
        #    m = re.match(r'^\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,[\u4e00-\u9fa5]+',line)
        #    if m == None :
...
def validate_point_inside_bounds(x,y,imWidth,imHeight):
    import numpy as np
    x = np.clip(x, 0, imWidth)
    y = np.clip(y, 0, imHeight)
    if(x<0 or x>imWidth):
...
def sort_clockwise(poly):
    ...
    poly_angles = [None, None, None, None]
    for i in range(4):
        x, y = poly_cented[i]
        poly_angles[i] = math.atan2(y, x)
...
def validate_clockwise_points(points):
    ...
    if summatory>0:
        points = [points[x] for x in [0,1,6,7,4,5,2,3]]
        point = [point[x] for x in [0,3,2,1]]
        summatory = - summatory
    #if summatory>0:
    ...
    import numpy as np
    import math
    cent = np.mean(point, axis=0)
    pt_cent = point - cent
    pt_angle = [math.atan2(p[1], p[0]) for p in pt_cent]
    point = [point[i] for i in np.argsort(pt_angle)]
    points = [point[0][0], point[0][1], point[1][0], point[1][1], point[2][0], point[2][1], point[3][0], point[3][1]]
